...
const configPath = path.resolve(__dirname, 'nextblocknew2.txt');
            ...
            for (var actionItem in actions) {
                ...
                for (var record in rwSet) {
                    ...
                    if (rwSet[record].namespace != 'lscc') {
                        ...
                        const logfilePath = path.resolve(__dirname, 'nextblocknew2.txt');
...
async function writeValuesToCouchDBP(nano, channelname, writeObject) {
    ...
    return new Promise((async (resolve, reject) => {
        ...
        try {
            ...
            const historydbname = channelname + '_' + writeObject.chaincodeid + '_test1historywithvalue';
            ...
            try {
                for (var sequence in values) {
                    ...
                    const tempvalue = keyvalue.value
                    keyvalue.value = JSON.parse(tempvalue)
