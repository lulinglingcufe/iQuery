...
type commodity struct {
	ObjectType    string `json:"docType"`       //docType is used to distinguish the various types of objects in state database
	commodityName string `json:"commodityName"` //the fieldtags are needed to keep case from bouncing around
...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	} else if function == "initcommodity" { //change owner of a specific marble
		return t.initcommodity(stub, args)
	} else if function == "delete" { //delete a marble
		return t.delete(stub, args)
	} else if function == "consumecommodity" { //read a marble
		return t.consumecommodity(stub, args)
	}
...
// ============================================================
// initcommodity - create a new commodity, store into chaincode state
// ============================================================
func (t *SimpleChaincode) initCommodity(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	var err error
	...
	// ==== Input sanitation ====
	fmt.Println("- start init commodity")
	if len(args[0]) <= 0 {
		return shim.Error("1st argument must be a non-empty string")
	}
	if len(args[1]) <= 0 {
	...
	commodityAsBytes, err := stub.GetState(commodityID)
	if err != nil {
		return shim.Error("Failed to get commodity: " + err.Error())
	} else if commodityAsBytes != nil {
		fmt.Println("This commodity already exists: " + commodityID)
		return shim.Error("This commodity already exists: " + commodityID)
	}
	...
	// ==== Create commodity object and marshal to JSON ====
	objectType := "commodity"
	commodity := &commodity{objectType, commodityName, commodityID, price, owner}
	commodityJSONasBytes, err := json.Marshal(commodity)
	if err != nil {
		return shim.Error(err.Error())
	}
	// === Save commodity to state ===
	err = stub.PutState(commodityID, commodityJSONasBytes)
	if err != nil {
		return shim.Error(err.Error())
	}

	// ==== commodity saved. Return success ====
	fmt.Println("- end init commodity")
	return shim.Success(nil)
}
...
// consume a commodity
// ===========================================================
func (t *SimpleChaincode) consumecommodity(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	commodityID := args[0]
	consumerName := args[1]
	fmt.Println("- start consumecommodity ", commodityID, consumerName)
	...
	if err != nil {
		...
		return shim.Error("commodity does not exist")
	}

	commodityToConsume := commodity{}
	err = json.Unmarshal(commodityAsBytes, &commodityToConsume) //unmarshal it aka JSON.parse()
	if err != nil {
		return shim.Error(err.Error())
	}
	...
	if consumerToConsume.Balance < commodityToConsume.Price {
		return shim.Error("Consumer does not have enough balance")
	}
	consumerToConsume.Balance = consumerToConsume.Balance - commodityToConsume.Price

	//Get commodity Owner
	ownerAsBytes, err := stub.GetState(commodityToConsume.Owner)
	if err != nil {
	...
	ownerToConsume.Balance = ownerToConsume.Balance + commodityToConsume.Price
	...
	err = stub.PutState(commodityToConsume.Owner, ownerJSONasBytes) //rewrite the Owner
	if err != nil {
		return shim.Error(err.Error())
	}
	fmt.Println("- end consumecommodity (success)")
	return shim.Success(nil)
}
