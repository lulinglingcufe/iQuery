...
module.exports.run = function() {
    ...
    let commodityID = '#supermarket'+ txIndex.toString();
    let price = Math.floor(Math.random()*10) + (Math.floor(Math.random()*10) * 9) ; //(0,100)
    let onwerID = Math.floor(Math.random()*10) + (Math.floor(Math.random()*10) * 9)  ;
    let owner = 'User_'+ onwerID.toString()+ '@supermarket.com';
